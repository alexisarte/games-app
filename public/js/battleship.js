...
function pollGame(currentGame) {
  let idInterval = setInterval(() => {
    fetch(
      `/battleship/get/${localStorage.getItem(
        'boardId'
...
function setBoard() {
  ...
  for (let i = 0; i < ROWS; i++) {
    for (let j = 0; j < COLUMNS; j++) {
      ...
      $(id).classList.contains('taken')
        ? (board[i][j] = $(id).classList.item($(id).classList.length - 1))
        : null;
    }
  }
  const data = {
...
function showPoster(game) {
  const end = document.createElement('div');
    ...
    game.winner === localStorage.getItem('playerId')
      ? 'GANASTE'
      : game.winner !== 'TIE'
      ? 'PERDISTE'
      : 'EMPATE';
...
window.onload = function () {
  ...
  function dragDrop() {
    ...
    if (
      ...
      isValid(this.id, isHorizontal, selectedShipIndex)
    ) {
      for (let i = 0; i < draggedShipLength; i++) {
        // resto a la columna del id actual el indice de donde tomo el barco para posteriormente marcar el square como visitado
        // marco como visitado avanzando hacia la derecha por eso el + i
        let index = `${this.id[0]}${
          parseInt(this.id[1]) - selectedShipIndex + i
        }`;
      ...
      for (let i = 0; i < draggedShipLength; i++) {
        let index = `${parseInt(this.id[0]) - selectedShipIndex + i}${
          this.id[1]
        }`;
        userSquares[parseInt(index)].classList.add('taken', shipClass);
      }
