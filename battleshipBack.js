...
function setBoard(boardId, playerId, board) {
  ...
  if (game && isBoardOk(board)) {
    ...
    if (
      (playerId === game.keys.player1Id && game.board1 === null) ||
      (playerId === game.keys.player2Id && game.board2 === null)
    ) {
      //Si es el P1
...
function move(boardId, playerId, square) {
  ...
  if (
    game &&
    ((playerId === game.keys.player1Id && game.turn === PLAY1) ||
...
function isValid(name) {
  if (name !== null)
    if (name[0] === 'X' || name[0] === 'F' || name === AGUA) return false;
  return true;
}
...
function getGame(boardId, playerId) {
  ...
  const game = games.find((e) => e.keys.boardId === boardId);
  return game.keys.player1Id === playerId
    ? clientData(game, PLAY1)
    : game.keys.player2Id === playerId
    ? clientData(game, PLAY2)
    : null;
}
